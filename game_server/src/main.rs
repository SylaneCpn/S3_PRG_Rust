...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("tcp server waiting for connections at {} on port '{}'",addr, tcp_port);
...
fn handle_connection(stream : TcpStream , state : Arc<Mutex<Vec<Player>>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let position = Point {x : id as i32 * 20 , y : id as i32 * 20};

    let client_data = SelfData {
...
fn send_players_data(players : & [Player] , id : usize , output : &mut TcpStream) -> Result<() , Box<dyn std::error::Error>> {
